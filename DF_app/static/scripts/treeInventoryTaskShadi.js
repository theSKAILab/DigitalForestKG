...
function renderInventoryFilterTable(rows) {
  ...
  rows.forEach((row, index) => {
    ...
    if (row.type === "Numeric") {
      minCell = `<input class="range-input" id="${row.key}_min" type="number" value="${row.min}" />`;
      maxCell = `<input class="range-input" id="${row.key}_max" type="number" value="${row.max}" />`;
    } else {
      minCell = "-";
      if (row.options.length > 0) {
        ...
        maxCell = `<input class="status-select" id="${row.key}_value" type="text" placeholder="Enter status..." />`;
      }
    }
...
function loadInventoryFilters() {
  ...
  Promise.all([...numericPromises, fetchTreeStatusOptions(year, treeName)])
    .then(rows => {
      inventoryRows = rows;
...
function applyInventoryFilters() {
  ...
  selectedInventoryFilters.forEach(filter => {
    if (filter.type === "Numeric") {
      formData.append(filter.key, document.getElementById(`${filter.key}_min`).value);
      formData.append(filter.key, document.getElementById(`${filter.key}_max`).value);
    } else {
      const statusValue = document.getElementById(`${filter.key}_value`).value;
      if (statusValue) formData.append(filter.key, statusValue);
    }
  });

  statusText.textContent = "Applying inventory filters...";

  fetch("/treeclassmap", { method: "POST", body: formData })
    .then(response => response.json())
    .then(data => {
