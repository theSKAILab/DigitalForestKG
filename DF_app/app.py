...
template_dir = os.environ.get("FLASK_TEMPLATES", "templates")
...
@app.route("/gr_query")
def graphical_query():
    return render_template("graphical_query.html", title="Select Query parameters")
...
def minmax():
    ...
    if region:
        ...
        wkt_text = (
            "'''<http://www.opengis.net/def/crs/EPSG/8.5/4326>%s'''^^<http://www.opengis.net/ont/geosparql#wktLiteral>"
            % wkt_geom
        )
...
def categorygroup():
    ...
    return jsonify(
        {"classlisthtml": render_template("itemList.html", class_list=class_list)}
    )
...
def query_result():
    ...
    initialize_map = folium.Map(
        location=[45.56528, -69.56528],
        zoom_start=7,
        max_lat=47.391892,
        max_lon=-66.793541,
        min_lat=42.694265,
...
def userpara():
    ...
    if minelevation_key in processeddata:
        ...
        FILTER (?minelevation >= %s && ?minelevation <= %s)""" % (
            sel_minelevation[0],
            sel_minelevation[1],
        )
    ...
    if maxelevation_key in processeddata:
        ...
        FILTER (?maxelevation >= %s && ?maxelevation <= %s)""" % (
            sel_maxelevation[0],
            sel_maxelevation[1],
        )
    ...
    if meanelevation_key in processeddata:
        ...
        FILTER (?meanelevation >= %s && ?meanelevation <= %s)""" % (
            sel_meanelevation[0],
            sel_meanelevation[1],
        )
    ...
    if meanslope_key in processeddata:
        ...
        FILTER (?meanslope >= %s && ?meanslope <= %s)""" % (
            sel_meanslope[0],
            sel_meanslope[1],
        )
    ...
    selected_region_table["geometry"] = selected_region_table["geometry"].apply(
        wkt.loads
    )
    ...
    filtered_region_table["geometry"] = filtered_region_table["geometry"].apply(
        wkt.loads
    )
    ...
    return jsonify(
        {
            "selected_region_geoj": selected_region_geoj,
            "filtered_region_geoj": filtered_region_geoj,
        }
    )
...
def family():
    ...
    return jsonify(
        {
            "familylisthtml": render_template(
                "treeInventoryTaskLoops.html", family_list=family_list
            )
        }
    )
...
def genus():
    ...
    return jsonify(
        {
            "genuslisthtml": render_template(
                "treeInventoryTaskLoops.html", genus_list=genus_list
            )
        }
    )
...
def species():
    ...
    return jsonify(
        {
            "specieslisthtml": render_template(
                "treeInventoryTaskLoops.html", species_list=species_list
            )
        }
    )
...
def treeclassmap():
    ...
    if "dbhrange" in processeddata:
        ...
        dbh_query = """?snapshot fio:includeRecord ?dbhrecord.
                       ?dbhrecord stad:hasQualityKind dfds:TreeDiameterAtBreastHeight;
                        ...
                        FILTER (?dbh >= %s && ?dbh <= %s && ?dbh != "NaN"^^xsd:float)""" % (
            sel_dbh[0],
            sel_dbh[1],
        )
    ...
    if "tahrange" in processeddata:
        ...
        tah_query = """?snapshot fio:includeRecord ?tahrecord.
                       ?tahrecord stad:hasQualityKind dfds:TreeActualHeight;
                        ...
                        FILTER (?tah >= %s && ?tah <= %s && ?tah != "NaN"^^xsd:float)""" % (
            sel_tah[0],
            sel_tah[1],
        )
    ...
    if "tthrange" in processeddata:
        ...
        tth_query = """?snapshot fio:includeRecord ?tthrecord.
                       ?tthrecord stad:hasQualityKind dfds:TreeTotalHeight;
                        ...
                        FILTER (?tth >= %s && ?tth <= %s && ?tth != "NaN"^^xsd:float)""" % (
            sel_tth[0],
            sel_tth[1],
        )
    ...
    if selected_env_prop:
        ...
        for result in selected_region_results["results"]["bindings"]:
            selected_region_dic["geometry"].append(
                result["geom"]["value"].split(">")[1]
            )
            selected_region_dic["cellID"].append(result["cellID"]["value"])
...
def categorygrouptree():
    ...
    return jsonify(
        {
            "classlisthtml": render_template(
                "treeInventoryTaskLoops.html", class_list=class_list
            )
        }
    )
...
def treeswithpreferences():
    ...
    return jsonify(
        {
            "treeslisthtml": render_template(
                "preferencesTaskLoop.html", trees_list=trees_list
            )
        }
    )
...
def feasibiltycheck():
    ...
    for index, roww in tolerance_range_table2.iterrows():
        ...
        data_gdf[variable] = data_gdf.apply(
            lambda row: range_check(row, index, lower, upper), axis=1
        )
